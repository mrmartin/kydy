...
GitHub Actions znovu sestaví nové srovnání z verzovaných dat a spustí regresní kontroly před publikováním. Automatické stažení či přepsání hodnocení při návštěvě webu neprobíhá. Metodika 3 hodnotí oporu jednotlivých slibů v pěti kategoriích a u kampaní ukazuje jejich skladbu. Nahrazuje původní prognózu ANO/MOŽNÁ/NE, která všude vycházela MOŽNÁ. Volební podmínky zůstávají v detailu. JSON schéma verze 3 používá `evidence_status` a `evidence_counts`, přehled výkonné odpovědnosti `governance` a konkrétní výtky `accountability`; CSV obsahuje i tyto podklady; původní `verdict` se již nepoužívá. Barvy netvoří žebříček ani záruku splnění.
