...
def prepare(archive):
    ...
    for party in packed['parties']:
        ...
        governance = dict(curation.GOVERNANCE[number])
        governance['references'] = references(governance['references'])
        ...
        for item in curation.CLAIMS:
            ...
            if accountability:
                ...
                accountability['references'] = references(accountability['references'])
            claims.append({'accountability': accountability, 'id': item['key'], 'title': item['title'], 'promise': item['promise'] or old['tvrzeni_parafraze'], 'source': references([(sid, item['locator'] or old['lokator'])])[0], 'evidence_status': item['evidence_status'], 'reason': item['why'], 'signal': item['signal'], 'history': history, 'authority': competence, 'practical': item['practical'], 'change': item['change'], 'counterevidence': item['counter'] or 'Budoucí závazek není tvrzení, že stejné opatření již bylo provedeno. Chybějící podklad nepovažujeme za důkaz opaku.', 'references': references(item['references']), 'authority_references': references(competence_refs), 'votes': vote_details, 'updated': curation.DATE, 'review': 'zdrojové posouzení a kontrola mezí interpretace; bez nezávislé externí recenze'})
        claims.sort(key=lambda c: c['id'])
        campaigns.append({'number': number, 'name': party['kratky_nazev'], 'full_name': party['listina'], 'leader': party['lidr'], 'candidates': party['kandidatu'], 'current_members': party['soucasnych_zastupitelu'], 'past_members': party['drzitelu_mandatu_2022_2026'], 'experience': experience, 'governance': governance, 'summary': editorial['summary'], 'selection': editorial['selection'], 'political': editorial['political'], 'political_references': references(editorial.get('political_refs', [])), 'program_references': references([(s, party['pokryti_kampane']) for s in party['zdroj_ids'].split(';')]), 'claims': claims})
    ...
    result = {'schema_version': 3, 'methodology_version': 3, 'assessment_question': 'Jakou oporu má tento slib?', 'updated': curation.DATE, 'campaign_snapshots': '2026-09-19', 'votes_through': '2026-06-23', 'polling': 'K 20. 9. 2026 nebyl v této rešerši ověřen použitelný aktuální brněnský volební model. Neznamená to, že žádný neexistuje. Mandáty neodhadujeme z celostátních průzkumů ani z volebního potenciálu.', 'coverage_note': 'Z9/37 ze dne 15. 9. 2026: zkontrolována stránka jednání a vybrané předložené materiály. V načteném indexu nebyly odkazy na zápis ani jmenné protokoly; návrhy nezaměňujeme za schválená rozhodnutí. Úplná ověřená hlasovací řada končí 23. 6. 2026.', 'campaigns': campaigns, 'sources': normalized_sources, 'changes': [{'date': curation.DATE, 'text': 'První vydání: oddělení slibů, doložených kroků, pravomocí a podmínek; zahrnuty starší role a zářijová koaliční vyjádření. Původní prognostická hodnocení byla všechna MOŽNÁ.'}, {'date': curation.DATE, 'text': 'Metodika 2: barva vyjadřuje oporu slibu, nikoli předpověď splnění. Čtyři kategorie, samostatné zařazení všech 57 slibů a skladba místo většinového verdiktu kampaně; stejné archivované podklady.'}, {'date': curation.DATE, 'text': 'Metodika 3: přidána kategorie Neudělali to, když mohli, přehled výkonné odpovědnosti všech kandidátek a dva úzce vymezené případy vlastního souhlasu s prodejem Dornychu; doplněny původní podklady o hlasování rady a uskutečnění prodeje.'}]}
    result['copy_updated'] = curation.COPY_UPDATED
    result['changes'].append({'date': curation.COPY_UPDATED, 'text': curation.COPY_CHANGE})
