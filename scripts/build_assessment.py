...
def build():
    ...
    assert d['schema_version'] == 2 and d['methodology_version'] == 2
    ...
    with (SITE/'data/hodnoceni-slibu.csv').open('w',encoding='utf-8-sig',newline='') as f:
        w=csv.writer(f,lineterminator='\n');w.writerow(['id','cislo_listiny','kampan','slib','opora_kod','opora_popis','duvod','dosavadni_jednani','pravomoc','proveditelnost','politicke_podminky','protidukazy','co_by_zmenilo_zaver','zdroj_url','lokator','dalsi_zdroje','hlasovani','overeno'])
        for p in campaigns:
            for c in p['claims']:
                w.writerow([c['id'],p['number'],p['name'],c['promise'],c['evidence_status'],STATUSES[c['evidence_status']]['label'],c['reason'],c['history'],c['authority'],c['practical'],p['political'],c['counterevidence'],c['change'],d['sources'][c['source']['source']]['url'],c['source']['locator'],';'.join(r['source'] for r in c['references']+c['authority_references']),';'.join(v['id'] for v in c['votes']),c['updated']])
